...
if ($_POST['action']=='I' || $_POST['action']=='U' || $_POST['action']=='T' || $_POST['action']=='R' || $_POST['action']=='A' || $_POST['action']=='IB' || $_POST['action']=='IS' || $_POST['action']=='IR' || $_POST['action']=='ICR') { 
		...
		$extension = substr(strrchr($_FILES['aym_fil']['name'][$i],'.'), 1);
			
		#VALIDACION SI LA RUTA DEL ARCHIVO ES VALIDA
		if (strlen($nombre_archivo_ori) < 4) {
			$ReturnStatus =1;
			$Msg = 'Debe especificar una ruta v&aacute;lida para el archivo <br /><br /> <strong>Sugerencia:</strong> <br /> Verifique los datos introducidos en el formulario.';
		...
		if ($tipo_archivo <> 'application/pdf') {
			$ReturnStatus =1;
			$Msg = 'El archivo <strong>'.$nombre_archivo_ori.'</strong> tiene una extensi&oacute;n NO v&aacute;lida. <br /><br /> <strong>Sugerencia:</strong> <br /> Guarde el archivo en formato. <br /><br /> .pdf ';
		...
		if (!move_uploaded_file($_FILES['aym_fil']['tmp_name'][$i],$aym_dir."".$fil_nam.".".$extension)) {
			$ReturnStatus =1;
