...
# ADRIAN LOPEZ RODRIGUEZ OCT/10/2022
	...
	$aym_sql = "CALL `AYM_SP_DELETE_PROFILE_FUNCTION`(".$_POST['pro_id'].");";
	
	if (!$aym_sql_delete_profile_function=mysqli_query($link,$aym_sql)) {
		# INCLUSIÓN --> COMPONENTE QUE MUESTRA LOS ERRORES DE SINTAXIS
		include_once $_SERVER['DOCUMENT_ROOT'].'/admin/aym_component/aym_message/aym_show_message_error.php';
	} else {
		$row_del_user_profile=mysqli_fetch_assoc($aym_sql_delete_profile_function); 
		$ReturnStatus=$row_del_user_profile['ReturnStatus'];
		$Msg=$row_del_user_profile['Msg'];
		mysqli_free_result($aym_sql_delete_profile_function);
	} 
