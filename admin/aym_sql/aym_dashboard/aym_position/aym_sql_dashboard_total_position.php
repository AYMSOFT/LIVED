...
# ANDRES CASTRO AGO/09/2023
	...
	# QUERY
    $aym_sql = "CALL `AYM_SP_DASHBOARD_TOTAL_POSITION`('".$_POST['com_id']."');"; 
    
	# QUERY --> LISTADO LOS REGISTROS 	
	if (!$aym_sql_dashboard_total_position=mysqli_query($link,$aym_sql)) {
		# INCLUSIÓN --> COMPONENTE QUE MUESTRA LOS ERRORES DE SINTAXIS
		include_once $_SERVER['DOCUMENT_ROOT'].'/admin/aym_component/aym_message/aym_show_message_error.php';	
	} else {	
        $row_total_position = mysqli_fetch_array($aym_sql_dashboard_total_position);
        mysqli_free_result($aym_sql_dashboard_total_position);
 	}
	mysqli_close($link);
?>       