...
<div class="aym_wrap_form">
	<form action="/aymcms/actionfaq" method="post" name="frm_add_faq" id="frm_add_faq" autocomplete="off" enctype="multipart/form-data">
		<fieldset>
			<div>
				...
				<select name="faq_cat_id" id="faq_cat_id">
					...
					?>
						<option value="<?= $row_list_faq_category['faq_cat_id'];?>"><?= $row_list_faq_category['faq_cat_nam']; ?></option>
					<?php } mysqli_free_result($aym_sql_list_faq_category);?>
				</select>
			</div>
			<div>
				<label for="faq_que" >Pregunta:</label>
				<input type="text" id="faq_que" name="faq_que">
			</div>
			<h3>Respuesta</h3>
			<div class="aym_frm_full medium">
				<div class="aym_ckeditor_toolbar"><div class="document-editor__toolbar"></div></div>
				<div class="aym_ckeditor_editor"><div name="content" id="editor" data-id="faq_ans"></div></div>
			</div>
			<input type="hidden" name="faq_ans" id="faq_ans"  value=''>
