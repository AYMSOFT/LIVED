<?php # **************************** AYMCMS V: 7.0 ********************
# FORMULARIO PARA LISTAR LAS IMAGENES
# © 2023, AYMSOFT SAS
# ADRIAN LOPEZ RODRIGUEZ ENE/31/2023
	...
	# INCLUSIÓN --> PROCEDIMIENTO QUE LISTA LAS SUBCATEGORIAS
...
<div class="aym_wrap_list">
	<div class="aym_wrap_filter">
		<form name="frm_list_gallery_image" data-module="gallery" data-file="list_gallery_image" id="frm_list_gallery_image" enctype="multipart/form-data">
			...
			<nav id="aym_show_filter">
				...
				<div>
					...
					<select name="gal_cat_id" id="gal_cat_id" class="aym_select">
						<?php while ($row_get_gallery_category =  mysqli_fetch_assoc($aym_sql_list_all_gallery_category)){?>
							<option value="<?=$row_get_gallery_category['gal_cat_id'];?>" <?=$row_get_gallery_category['gal_cat_id']==$_GET['gal_cat_id']?'selected':'';?>><?=$row_get_gallery_category['gal_cat_nam'];?></option>
						<?php } mysqli_free_result($aym_sql_list_all_gallery_category);?>
					</select>
				</div>
				<div>
					<label for="gal_sub_cat_id">Subcategoría</label>
					<select name="gal_sub_cat_id" id="gal_sub_cat_id" class="aym_select">
						<?PHP while ($row_get_gallery_subcategory =  mysqli_fetch_assoc($aym_sql_list_all_gallery_subcategory)){?>
							<option value="<?=$row_get_gallery_subcategory['gal_sub_cat_id'];?>"><?=$row_get_gallery_subcategory['gal_sub_cat_nam'];?></option>
	...
	<div id="aym_list_gallery_image"></div>
